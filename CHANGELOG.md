...
- Canary testers can submit test reports (`POST /versions/{id}/test-reports`), and approval is blocked while reported leaks are open.
- The Go control plane (phase D): `mp-controlplane` (gRPC deploy, rollback, and kill switch, sharing the audit log), `mp-gateway` (subdomain → organization, login check at the edge, killed models blocked), and the `mp` CLI. The backend deploys through it.
